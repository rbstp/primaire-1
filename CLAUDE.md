...
- **Le minifig bouge d'un bloc.** Pas d'animation propre à une partie du corps: une `.animation` sur la tête la faisait partir avant le corps. Torse, hanches et jambes sont une seule silhouette, le katana est tenu dans la main droite, devant. Seule exception: sur l'écran d'ouverture il fait tourner son katana dans la main (`swordAngle`, piloté par `withAnimation` depuis `WeekPickerView`), un tour complet par le bas pour ne jamais balayer le visage.
...
- **Une lettre se dessine comme à l'école, partout.** `GlyphMark` trace les lettres et les chiffres avec les traits de `GlyphLibrary` au lieu de la police: SF Pro dessine un `a` à deux étages, et sur une tuile à côté du `a` qu'il calligraphie, ce sont deux lettres différentes pour lui. Ce que la bibliothèque ne couvre pas garde la police, et un prénom entier aussi, parce qu'il se reconnaît comme une forme et que la moitié de ses lettres n'ont pas de tracé.
...
Écrans: `dojo`, `letters`, `numbers`, `names`, `build`, `vowels`, `alphabet`, `trace`, `log`, `summary`, `parent`, `glyphs`, `pictures` (les objets du jeu des voyelles), `scenes` (les trois célébrations de fin). Un écran lancé ainsi est la racine de la pile: il n'a pas de bouton de retour.
...
- Le sens du trait des accents (`é`, `è`) suit la lecture, de gauche à droite. À vérifier auprès de l'enseignante, comme le 4, le 5 et le 8.
